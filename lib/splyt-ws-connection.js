...
module.exports = class SplytWSConnection extends CoEventEmitter {
  ...
  constructor(url, options = {}) {
    ...
    this.messageHandler = new WSMessageHandler({
      compress: ('compress' in options) ? options.compress : true,
    });

    this.messageHandler.on('request', (ctx) => {
      return this.call('request', ctx.incoming.payload.method, ctx.incoming.payload.data, ctx.socket);
    });
    this.messageHandler.on('push', (ctx) => {
      return this.call('push', ctx.incoming.payload.method, ctx.incoming.payload.data, ctx.socket);
    });
  }
  ...
  _connect() {
    ...
    ws.on('message', (message) => {
      this.messageHandler.incoming({
        socket: ws,
        incoming: new WSMessageHandler.Incoming(message),
      });
    });
   ...
   */
  request(method, data = {}) {
    return this.messageHandler.outgoing({
      socket: this[WS$],
